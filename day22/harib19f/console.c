...
int cmd_app(struct CONSOLE *cons,int *fat,char *cmdline){
	...
	if(file_info!=0){	//找到文件的情况下
		p = (char *) memman_alloc_4k(memman, file_info->size);
		file_LoadFile(file_info->clustno, file_info->size, p, fat, (char *) (ADR_DISKIMG + 0x003e00));
		if (file_info->size >= 36 && strncmp(p + 4, "Hari", 4) == 0) {
			segSize=*((int *)(p+0x0000));				//数据段的大小根据.hrb文件中指定的值进行分配
			esp=*((int *)(p+0x000c));
			dataSize=*((int *)(p+0x0010));
			dataHrb=*((int *)(p+0x0014));				//将.hrb文件中的数据部分先复制到数据段再启动程序
			q=(char*) memman_alloc_4k(memman,segSize);	//给应用程序分配segSize大小的存储空间
			*((int *)0xfe8)=(int) q;					//由于无法从cmd_app向hrb_api直接传递数据，因此只好又在内存里找个地方存放一下了。
			// hlt.hrb读入内存之后，将其注册为GDT的1003号。因为1~2号有dscbl.c使用，3~1002由mtask.c使用
			set_segmdesc(gdt + 1003, file_info->size - 1, (int) p, AR_CODE32_ER+0x60);	//x86架构中，在段定义的地方，如果将访问权限加上0x60后，就可以将段设置为应用程序用
			set_segmdesc(gdt + 1004, segSize-1, (int) q			,  AR_DATA32_RW+0x60);	//应用程序的代码段：1003*8 数据段1004*8
			...
			console_putstr0(cons,".hrb file format error!\n");
		}
		memman_free_4k(memman, (int) p, file_info->size);
		console_newline(cons);
		return 1;
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax){
	...
	struct SHTCTL *shtctl=(struct SHTCTL*)*((int *) 0x0fe4);	//shtctl的值是bootpack.c的HariMain中的变量
	struct SHEET *sht;
	...
	if(edx==1){
		...
		console_putstr0(cons,(char*) ebx+ds_base);
		
	}
	else if(edx==3){
		console_putstr1(cons,(char *) ebx+ds_base,ecx);
	}
	else if(edx==4){
		...
		sheet_setbuf(sht,(char*) ebx+ds_base,esi,edi,eax);	//EBX窗口缓冲区
...
int *inthandler0d(int *esp){
	...
	sprintf(s,"EIP=%08X\n",esp[11]);	//将ESP(即栈)的11号元素(即EIP)显示出来
	console_putstr0(cons,s);
