...
//	sheet_setbuf(sht_console, buf_console, 256, 165, -1); /* 摟柧怓側偟 */
//	make_window8(buf_console, 256, 165, "consoleole", 0);
		...
		if(fifo32_status(&task->fifo)==0){	//没有输入的时候就把这个进程设置为休眠状态
			...
			if(i>=256&&i<=511){		//键盘任务（通过任务A）
				if(i==8+256){
					...
					//执行命令
					if(cmdline[0]=='m'&&cmdline[1]=='e'&&cmdline[2]=='m'&&cmdline[3]==0){	//判断是“mem”命令
