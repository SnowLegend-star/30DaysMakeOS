...
void HariMain(void)
{
	...
	buf_console=(unsigned char*)memman_alloc_4k(memman,512*165);
	sheet_setbuf(sht_console,buf_console,512,165,-1);	//无透明色
	make_window8(buf_console,512,165,"console",0);
	make_textbox8(sht_console,8,28,240+256,128,COL8_000000);
...
void console_task(struct SHEET *sheet,unsigned int memtotal){
	...
	//显示提示符
	putfonts8_asc_sht(sheet,8,28,COL8_FFFFFF,COL8_000000,"please use 01~26 to represent a~z",27);
			...
			if(i>=256&&i<=511){		//键盘任务（通过任务A）
				if(i==8+256){
					...
					if(strcmp(cmdline,"130513")==0){	//mem命令
						...
						sheet_refresh(sheet,8,28,8+240+256,28+128);	//覆盖整个页面
						cursor_y=28;
					}
						...
						for(x=0;x<224;x++){
							...
							if(file_info[x].name[0]!=0xe5){ //0xe5表示这个文件已经被删除了
								if((file_info[x].attribute&0x18)==0){
									sprintf(s,"filename.ext   %7d",file_info[x].size);
									for(y=0;y<8;y++){
...
int cons_newline(int cursor_y,struct SHEET *sheet){
	...
	if(cursor_y<28+112){
		...
		sheet_refresh(sheet,8,28,8+240+256,28+128);
	}
	return cursor_y;
}
