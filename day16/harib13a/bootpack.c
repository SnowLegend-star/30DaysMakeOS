...
void HariMain(void)
{
	...
	task_init(memman);	//没有接收返回值的参数吗
	task_b=task_alloc();
	task_b->tss.esp=memman_alloc_4k(memman,64*1024)+64*1024-8;		//专门为任务B定义的栈
	...
	task_b->tss.fs = 1 * 8;
	task_b->tss.gs = 1 * 8;
	*((int *) (task_b->tss.esp+4))=(int) sht_back;	//此举是为了把sht_back从任务A传到任务B
	task_run(task_b);
