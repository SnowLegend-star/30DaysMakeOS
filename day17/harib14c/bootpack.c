...
void console_task(struct SHEET *sheet){
	...
	int i,fifobuf[128],cursor_x=8,cursor_c=COL8_000000;	//cursor_x用来记录光标显示位置的变量，输入一个字符这个变量就加8 cursor_c表示光标颜色，0.5s变化一次
	fifo32_init(&fifo,128,fifobuf,task);

	timer=timer_alloc();
	timer_init(timer,&fifo,1);	//又光初始化而不赋值是吧
	timer_settime(timer,50);

	while(1){
		io_cli();
		if(fifo32_status(&fifo)==0){	//没有输入的时候就把这个进程设置为休眠状态
			task_sleep(task);	
			io_sti();
		}
		else{
