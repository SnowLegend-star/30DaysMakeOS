...
void console_task(struct SHEET *sheet, unsigned int memtotal)
{
	...
	char cmdline[30];
	cons.sht = sheet;
	cons.cur_x = 8;	//cur_x用来记录光标显示位置的变量，输入一个字符这个变量就加8 cur_c表示光标颜色，0.5s变化一次
	cons.cur_c = -1;	//因为一开始是切换到任务A
	cons.cur_y = 28; 	// 原来28是光标高啊 按下Enter纵坐标加1
	*((int *) 0x0fec)=(int) &cons;
...
void cons_putchar(struct CONSOLE *cons,int letter,char move){
	char s[2];
	s[0]=letter;
	s[1]=0;
	if (s[0] == 0x09)
...
void cons_runcmd(char *cmdline,struct CONSOLE *cons,int *fat,unsigned int memtotal){
	...
	else if (strncmp(cmdline, "1111", 4) == 0) // type命令 显示文件内容	
	{
	...
	{ 	
		if(cmd_app(cons,fat,cmdline)==0){// 不是命令，不是应用程序，也不是空行
			// putfonts8_asc_sht(cons->sht, 8, cons->cur_y, COL8_FFFFFF, COL8_000000, "Wrong command!", 15);
			// cons_newline(cons);
			// cons_newline(cons);
...
void cmd_type(struct CONSOLE *cons, int *fat, char *cmdline){
	...
	struct FILEINFO *file_info = file_search(cmdline + 4, (struct FILEINFO *) (ADR_DISKIMG + 0x002600), 224);
	...
	{ // 找到文件的情况下
		...
		cons_putstr1(cons,p,file_info->size);
...
int cmd_app(struct CONSOLE *cons,int *fat,char *cmdline){
	...
	if(file_info!=0){
		...
		file_loadfile(file_info->clustno, file_info->size, p, fat, (char *) (ADR_DISKIMG + 0x003e00));
		// hlt.hrb读入内存之后，将其注册为GDT的1003号。因为1~2号有dscbl.c使用，3~1002由mtask.c使用
		set_segmdesc(gdt + 1003, file_info->size - 1, (int) p, AR_CODE32_ER);
		farcall(0, 1003 * 8);
		memman_free_4k(memman, (int) p, file_info->size);
		cons_newline(cons);
		return 1;
...
void cons_putstr1(struct CONSOLE *cons,char *s,int l){	//l表示将要显示的字符串长度
	int i;
	for(i=0;i<l;i++){
