...
void console_runcmd(char *cmdline,struct CONSOLE *cons,int *fat,unsigned int memtotal){
	...
	{ 	
		if(cmd_app(cons,fat,cmdline)==0){// 不是命令，不是应用程序，也不是空行
			putfonts8_asc_sht(cons->sht, 8, cons->cursor_y, COL8_FFFFFF, COL8_000000, "Wrong command!", 15);
			console_newline(cons);
			console_newline(cons);			
		}
	...
	for (i = 0; i < 224; i++) {
		...
		if (file_info[i].name[0] != 0xe5) {	// 0xe5表示这个文件已经被删除了
			if ((file_info[i].attribute & 0x18) == 0) {
				sprintf(s, "filename.ext   %7d", file_info[i].size);	//// 原来filena.ext是保留字啊
				for (j = 0; j < 8; j++) {
...
void cmd_type(struct CONSOLE *cons, int *fat, char *cmdline){
	struct MEMMAN *memman = (struct MEMMAN *) MEMMAN_ADDR;
	struct FILEINFO *file_info = file_Search(cmdline + 5, (struct FILEINFO *) (ADR_DISKIMG + 0x002600), 224);
	char *p;
	int i;
	if (file_info!=0)
	{ // 找到文件的情况下
...
int cmd_app(struct CONSOLE *cons,int *fat,char *cmdline){
	...
	if(i%2==1)	//假如命令行输入的数字个数为奇数，这种格式是非法的，直接返回
		return 0;
